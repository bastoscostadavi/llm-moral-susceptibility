...
\end{equation}
Our robustness index is the reciprocal of that average:
\begin{equation}
  R = \frac{1}{\bar{u}},\qquad \sigma_R = \frac{\sigma_{\bar{u}}}{\bar{u}^2},
  \label{eq:robustness}
\end{equation}
with uncertainty propagated from the $\sigma_{\overline{u}}$.
...
\end{align}
From $s_{qg}$ we obtain a group-level susceptibility samples
\begin{equation}
  S_g = \frac{1}{|\mathcal{Q}|} \sum_{q \in \mathcal{Q}} s_{qg}.\label{eq:group-susceptibility}
\end{equation}
Then, the reported susceptibility is the mean over groups
\begin{equation}
  S = \frac{1}{G} \sum_{g=1}^{G} S_g,\qquad \sigma_S = \sqrt{\frac{1}{G(G-1)} \sum_{g=1}^{G} (S_g - S)^2}
  \label{eq:overall-susceptibility}
\end{equation}
with its standard error estimated from the between-group variability.

Foundation-specific susceptibilities reuse Eqs.~\eqref{eq:question-dispersion}--\eqref{eq:overall-susceptibility} after restricting \(\mathcal{Q}\) to the question subset \(\mathcal{Q}_f\) for foundation \(f\). Our results are displayed in Fig~\ref{fig:susceptibility-foundations}.
